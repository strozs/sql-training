...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT ROUND(SUM(budget_adj), 2) AS total_budget, 
                            ROUND(SUM(revenue_adj), 2) AS total_revenue FROM movies`;
  ...
  it(
    ...
    async done => {
      const query = `SELECT SUBSTR(release_date, 1, 4) AS year, COUNT(*) AS count
                    FROM movies
  ...
  it(
    ...
    async done => {
      const query = `SELECT user_id, COUNT(rating) AS count
                    FROM movie_ratings
                    GROUP BY user_id
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(rating) AS count, substr(time_created, 6, 2) AS month
                    FROM movie_ratings
                    GROUP BY substr(time_created, 6, 2)
                    ORDER BY COUNT(*) DESC `;
      const result = await db.selectMultipleRows(query);
